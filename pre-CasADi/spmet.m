...
p.R_s_p = SX.sym('p.R_s_p');
p.ElecFactorDA = SX.sym('p.ElecFactorDA');
p.epsilon_e_n = SX.sym('p.epsilon_e_n');
p.t_plus = SX.sym('p.t_plus');
...
% t_pulse = 240;
% % t = 0:p.delta_t:(t_pulse);
% I = 4*p.OneC*ones(size(t));

% I1 = 4*p.OneC*ones(size(t));
% I2 = -4*p.OneC*ones(size(t));
% I = horzcat(I1,I2);
% NT = 5;
% for i = 1:NT
%     I = horzcat(I,I);
% end
% t = 0:p.delta_t:length(I);

%%%%%%%%%%%%%%% DYNAMIC CHARGE/DISCHARGE CYCLES FROM EXPERIMENTS %%%%%%%%%%%%%%%
% path = 'input-data/';
% filename = 'US06x3_batt_ObsData';
% filetype = '.mat';
% load(horzcat(path,filename,filetype));
% 
% I = I/p.Area*10;
...
% p.delta_t = t(2)-t(1);
path = 'input-data/';
filename = 'UDDSx2_batt_ObsData.mat';
load(horzcat(path,filename))
...
% write data to csv
% c_n_data = horzcat(t,c_n);
% c_p_data = horzcat(t,c_p);
% dlmwrite('c_n_DC1.csv', c_n_data, 'delimiter', ',', 'precision', 16); 
% dlmwrite('c_p_DC1.csv', c_p_data, 'delimiter', ',', 'precision', 16); 

%%% Save output simulation data for use in coupled observers script
% sim_data = [t I V c_n c_p];
% save(horzcat(filename,'_sim_data'),'sim_data');
