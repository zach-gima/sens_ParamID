...
%   Edited for CasADi implementation April 11, 2019 by Zach Gima, based on
%   DFN CasADi implementation by Saehong Park
...
function [x_dot,x_outs,L,alg_states] = ode_spmet_casadi(x,cur,p)
    ...
    % cur = lininterp1f(data.time,data.cur,t,[]); % CASADI CHANGE

    %%% Parse dynamic states
    c_s_n = x(1:(p.Nr-1));
    ...
    %%% CASADI CHANGE -- not compatible with mean function
    % cen_bar = mean(c_ex(1:p.Nxn+1,:));
    % ces_bar = mean(c_ex((p.Nxn+1):(p.Nxn+p.Nxs+1),:));
    % cep_bar = mean(c_ex((p.Nxn+p.Nxs+1):(p.Nxn+p.Nxs+p.Nxp+1),:));

    cen_bar = casadi_mean(c_ex(1:p.Nxn+1,:));
    ces_bar = casadi_mean(c_ex((p.Nxn+1):(p.Nxn+p.Nxs+1),:));
    cep_bar = casadi_mean(c_ex((p.Nxn+p.Nxs+1):(p.Nxn+p.Nxs+p.Nxp+1),:));
...
%     kap_n_ref = p.ElecFactorK*electrolyteCond(T1,cen_bar);
%     kap_s_ref = p.ElecFactorK*electrolyteCond(T1,ces_bar);
%     kap_p_ref = p.ElecFactorK*electrolyteCond(T1,cep_bar);
...
%     kap_p = kap_p_ref * exp(p.E.kappa_e/p.R*(1/p.T_ref - 1/T1));
    ...
    [i_0n,i_0p] = exch_cur_dens(p,c_ss_n,c_ss_p,c_ex); % CASADI Change -- pass in c_ex instead of c_e_bar
...
%     dD_ep = dD_ep0 * Arrh_De;
    ...
    %% Concatenate time derivatives and state information
    x_dot = [c_s_n_dot; c_s_p_dot; c_e_dot; T1_dot; T2_dot; delta_sei_dot];
    x_outs = [c_s_n; c_s_p; c_e; T1; T2; delta_sei];
