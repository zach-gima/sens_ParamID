...
% inv_N2n = solve(N2n,SX.eye(N2n.size1()));
% inv_N2p = solve(N2p,SX.eye(N2p.size1()));

inv_N2n = inv(N2n);
inv_N2p = inv(N2p);


F1n = M1n - M2n*(inv_N2n*N1n);
F2n = M3n;
Gn = M4n - M2n*(inv_N2n*N3n);

F1p = M1p - M2p*(inv_N2p*N1p);
F2p = M3p;
Gp = M4p - M2p*(inv_N2p*N3p);
