...
function [F1n,F1p,F2n,F2p,F3n,F3p] = i_e_mats(p,SensFlag)
...
% M2 : i_e z
if SensFlag == 1
    M2n = SX.zeros(p.Nxn-1,2);
    M2p = SX.zeros(p.Nxp-1,2);
else
    M2n = zeros(p.Nxn-1,2); % CASADI change
    M2p = zeros(p.Nxp-1,2); % CASADI change
end
M2n(1,1) = -alpha_n;
M2n(end,end) = alpha_n;
